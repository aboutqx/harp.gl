...
export class MapView extends THREE.EventDispatcher {
    ...
    private renderTileObjects(tile: Tile, zoomLevel: number) {
        ...
        if (tile.willRender(zoomLevel)) {
            for (const object of tile.objects) {
                ...
                this.adjustRenderOrderForFallback(object, tile);
    ...
    private adjustRenderOrderForFallback(object: TileObject, tile: Tile) {
        ...
        if (object.userData.technique?.name === "extruded-polygon") {
            // Don't adjust render order for extruded-polygon b/c it's not flat.
            return;
        }

        if (object.userData.technique?._category?.startsWith("road") === true) {
